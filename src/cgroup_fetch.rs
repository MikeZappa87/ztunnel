...
fn find_first_cri_containerd_scope(dir: &Path) -> Result<PathBuf, CgroupErr> {
    let mut entries = fs::read_dir(dir)
        .map_err(|e| CgroupErr::ReadDir { dir: dir.to_path_buf(), source: e })?
        .filter_map(|res| res.ok())
        .collect::<Vec<_>>();

    // Deterministic: sort by name before picking "first".
    ...
    for entry in entries {
        ...
        if name.starts_with("cri-containerd-") && name.ends_with(".scope") {
            let p = entry.path();
            let md = fs::metadata(&p).map_err(|e| CgroupErr::Stat { path: p.clone(), source: e })?;
            if md.is_dir() {
                return Ok(p);
            }
        }
    ...
    Err(CgroupErr::NotFound { dir: dir.to_path_buf() })
}
