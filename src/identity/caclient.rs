...
impl CaClient {
    ...
    async fn fetch_certificate(&self, id: &CompositeId<RequestKeyEnum>) -> Result<tls::WorkloadCertificate, Error> {
                ...
                if self.enable_impersonated_identity {
                    Some(Struct {
                        fields: BTreeMap::from([(
                            ...
                            prost_types::Value {
                                kind: Some(Kind::StringValue(id.id().to_string())),
                            },
                        )]),
                    })
                } else {
        ...
        if self.enable_impersonated_identity && certs.identity().as_ref() != Some(id.id()) {
            error!("expected identity {:?}, got {:?}", id.id(), certs.identity());
...
mod tests {
    ...
    use crate::{
        identity::{Error, Identity}, test_helpers, tls, xds::istio::ca::IstioCertificateResponse
    };
