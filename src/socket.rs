...
pub mod socket_tests {
    ...
    async fn test_keep_alive_options() {
        ...
        assert_eq!(sock.tcp_keepalive_time().unwrap(), std::time::Duration::from_secs(10));
        assert_eq!(sock.tcp_keepalive_retries().unwrap(), 5);
        assert_eq!(sock.tcp_keepalive_interval().unwrap(), std::time::Duration::from_secs(2));
        let ut = cfg.keepalive_time + cfg.keepalive_retries * cfg.keepalive_interval;
